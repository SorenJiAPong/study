// 政治选择题每日抽考生成器 - 2026-08-29
// 确定性：以 DATE 数字为 mulberry32 种子，跨天新鲜、当天稳定。
const fs = require("fs");
const path = require("path");

const DATE = "20260829";
const OUT = "F:/wokebuddy创造/2026-07-29-19-58-13/政治每日抽考/每日抽考_2026-08-29.md";
...
const singles = uniq.filter(q => q.type === "single" || (q.type !== "multi"));
const multis = uniq.filter(q => q.type === "multi");
// 以防万一：single 取 type!=="multi"
...
function sample(arr, n, taken) {
  const avail = arr.filter((_, i) => !taken.has(i));
  const picked = [];
  const localTaken = new Set();
  while (picked.length < n && localTaken.size < avail.length) {
    const r = Math.floor(rnd() * avail.length);
    if (localTaken.has(r)) continue;
    localTaken.add(r);
    picked.push(avail[r]);
  }
  return picked;
...
// 先抽多选，再抽单选（用索引避免重复同一题）
const multiIdx = new Set();
const chosenMulti = sample(multis, N_MULTI, multiIdx).map(q => {
  multiIdx.add(multis.indexOf(q)); return q;
});
const singleIdx = new Set();
const chosenSingle = sample(singlePool, N_SINGLE, singleIdx).map(q => {
  singleIdx.add(singlePool.indexOf(q)); return q;
});
